...
<html>
	...
	<body>
		...
		<div class="tester-control-container">
				<%	
					createConn("varify");
				 	rs= st.executeQuery("select * from users where uname='"+request.getSession().getAttribute("fulluname")+"' ") ; 
				 	if(rs.next()){
				 %>
				...
				<div class="card card-container" style="">
    						...
    						<div class="card-body">
    							<h4><%= rs.getString("uname") %></h4>
      							...
      							<h5 class="">e-mail : <span style="color:grey"><%= rs.getString("email")  %></span></h5>
    						</div>
  				</div>
  				...
  				<%	
				 			...
				 			String[] orderID=null;
				 			if(Optional.ofNullable(rs.getString("orderids")).isPresent())
				 				orderID = HelperMethods.getOrderID(rs.getString("orderids"));
				 			else{
